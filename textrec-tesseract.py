...
class MyApp(QMainWindow, Ui_MainWindow):
    ...
    def process_shot(self, shot):
        ...
        print('pil img type: ', type(pil_img))
        buffer.close()

        ## 需要安装tesseract实现
        try:
            result = pytesseract.image_to_string(
                pil_img, timeout=60, lang='chi_sim')
            print(result)
            self.ocrtext.setPlainText(result)
        except RuntimeError as error:
            print(f"ERROR: An error occurred when trying to process the image: {error}")
            self.notify(f"An error occurred when trying to process the image: {error}")
            return
